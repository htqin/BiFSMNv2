...
class SeqEntityScore(object):
    ...
    def result(self):
        class_info = {}
        origin_counter = Counter([x[0] for x in self.origins])  # origins: label
        found_counter = Counter([x[0] for x in self.founds])  # founds: pred
        right_counter = Counter([x[0] for x in self.rights])  # each tags
