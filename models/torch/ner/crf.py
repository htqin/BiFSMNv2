...
class CRF(nn.Module):
    ...
    def _viterbi_decode(self, feats):
        ...
        best_path.reverse()
        out = [best_scores, best_path, scores]
        return out
    ...
    def _score_sentence(self, feats, tags, lens_):
        ...
        for i in range(feats.shape[0]):
            ...
            score[i] = torch.sum(
                self.transitions[pad_stop_tags[i, :lens_[i] + 1],
    ...
    def _obtain_identity(self, feature, input_lens):
        tags = []
        all_tags = []
        for feats, length in zip(feature, input_lens):
            confidences, tag_seq, scores = self._viterbi_decode(feats[:length])
            tags.append([tag for tag in tag_seq])
            all_tags.append(
                [[score_id for score_id, score in enumerate(score_dist)]
                 for score_dist in scores])
        return tags

    def _obtain_labels_from_identity(self, identity, id2label):
        tags = []
        for sample in identity:
            tags.append([id2label[i] for i in sample])
        return tags
